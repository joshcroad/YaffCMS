...
class Yaff {
	 ...
	 * Constructor.
	 *
	 * Gathers URL and retrieves .txt file accordingly. Parses through to the
	...
	public function __construct() {
		...
		if (isset($y['base_url']) && $y['base_url']) {
			...
			is_dir($file) ? $file = PAGES_DIR . $url . '/index.txt' : $file .= '.txt';

			if (file_exists($file)) {
				$y['page_content'] = file_get_contents($file);
			} else {
				$y['page_content'] = file_get_contents(PAGES_DIR . '404.txt');
				header($_SERVER['SERVER_PROTOCOL'] . ' 404 Not Found');
			}

			$this->get_page_meta();
			$y['page_content'] = preg_replace('#/\*.+?\*/#s', '', $y['page_content']);
			$this->extract_vars();
	...
	/**
	 * Retrieves the meta information from a .txt page file and adds the fields
	...
	/**
	 * Extracts variables from a .txt page file. If the user types %hello%,
